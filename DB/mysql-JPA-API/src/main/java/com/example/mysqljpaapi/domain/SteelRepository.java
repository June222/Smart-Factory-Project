...
//    @Query(value = "SELECT s FROM Steel s JOIN ")


//    @Query(value = "SELECT s FROM Steel s " +
//            "WHERE s.isNormal = FALSE AND " +
//            "s.localDateTime BETWEEN :t1 AND :t2 " +
//            "" +
//            "SELECT sl FROM SteelLabel sl " +
//            "WHERE sl.label = :label")
    ...
    List<Steel> findSteelsByLabelAndAndLocalDateTime(@Param("t1") LocalDateTime t1, @Param("t2") LocalDateTime t2, @Param("label") int label);
}
